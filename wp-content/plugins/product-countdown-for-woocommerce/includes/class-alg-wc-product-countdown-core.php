...
class Alg_WC_Product_Countdown_Core {
	...
	function get_variation_prices_hash( $price_hash, $_product, $display ) {
		$price_hash['alg_wc_product_countdown'] = serialize( array(
	 ...
	 * @version 1.4.1
	 * @since   1.0.0
	 * @todo    [feature] deprecate placeholders and replace them with shortcodes (e.g. `[alg_wc_ptc_days min_width="2" hide_if_zero="yes" before_singular=" " after_singular=" day" before_plural=" " after_plural=" days"]`)
	...
	function get_time_left_formatted( $product_id ) {
		if ( ( $result = $this->get_time_left( $product_id ) ) <= 0 ) {
			...
			if ( 'no' === get_option( 'alg_wc_product_countdown_format_human_time_diff', 'no' ) ) {
				$upper_period = get_option( 'alg_wc_product_countdown_time_format_upper_limit', 'hours' );
				switch ( $upper_period ) {
					case 'seconds':
						$seconds = $result;
						break;
