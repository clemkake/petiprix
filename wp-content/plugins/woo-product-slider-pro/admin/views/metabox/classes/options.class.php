 ...
 * @version 1.0.0
 */
abstract class SP_WPSP_Framework_Options extends SP_WPSP_Framework_Abstract {
	...
	public function element_debug() {
		...
		if ( ( isset( $this->field['debug'] ) && $this->field['debug'] === true ) || ( defined( 'SP_OPTIONS_DEBUG' ) && SP_OPTIONS_DEBUG ) ) {
			...
			$out .= '<pre>';
			$out .= '<strong>' . __( 'CONFIG', 'woo-product-slider-pro' ) . ':</strong>';
			$out .= "\n";
			ob_start();
			var_export( $this->field );
			$out .= htmlspecialchars( ob_get_clean() );
			$out .= "\n\n";
			$out .= '<strong>' . __( 'USAGE', 'woo-product-slider-pro' ) . ':</strong>';
			$out .= "\n";
			$out .= ( isset( $this->field['id'] ) ) ? "sp_get_option( '" . $this->field['id'] . "' );" : '';

			if ( ! empty( $value ) ) {
				$out .= "\n\n";
				$out .= '<strong>' . __( 'VALUE', 'woo-product-slider-pro' ) . ':</strong>';
				$out .= "\n";
				ob_start();
				var_export( $value );
				$out .= htmlspecialchars( ob_get_clean() );
		...
		if ( ( isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true ) || ( defined( 'SP_OPTIONS_DEBUG_LIGHT' ) && SP_OPTIONS_DEBUG_LIGHT ) ) {
			...
			$out .= ( isset( $this->field['id'] ) ) ? "sp_get_option( '" . $this->field['id'] . "' );" : '';
			$out .= "\n";
			$out .= '<strong>' . __( 'ID', 'woo-product-slider-pro' ) . ':</strong>';
			$out .= "\n";
		...
		if ( is_array( $this->multilang ) ) {
			...
			foreach ( $this->multilang['languages'] as $key => $val ) {
				...
				if ( $key != $this->multilang['current'] ) {
					...
					$class   = 'SP_WPSP_Framework_Option_' . $this->field['type'];
			...
			$out .= '</fieldset>';
			$out .= '<p class="sp-text-desc">' . sprintf( __( 'You are editing language: ( <strong>%s</strong> )', 'woo-product-slider-pro' ), $this->multilang['current'] ) . '</p>';
		...
		switch ( $type ) {
			...
			case 'product':
				...
				$all_product = $wpdb->get_results(
					'
			SELECT ID,post_title FROM `' . $wpdb->prefix . "posts` where post_type='product' and post_status = 'publish'"
				);

				if ( ! is_wp_error( $all_product ) && ! empty( $all_product ) ) {
					foreach ( $all_product as $product ) {
						$options[ $product->ID ] = '#' . $product->ID . ' ' . $product->post_title;
					}
				}
