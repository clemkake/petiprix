 ...
 * @version 1.0.0
 */
class SP_WPSP_Framework_Metabox extends SP_WPSP_Framework_Abstract {
	...
	public static function instance( $options = array() ) {
		if ( is_null( self::$instance ) && SP_WPSP_ACTIVE_METABOX ) {
	...
	public function render_meta_box_content( $post, $callback ) {
		...
		$transient    = get_transient( 'wpsp-metabox-transient' );
		...
		echo '<div class="sp-wpsp-framework sp-metabox-framework">';
		...
		if ( $the_current_post_type == 'sp_wpsp_shortcodes' ) {
			?>
			<div class="sp-wpsp-mbf-banner">
				<div class="sp-wpsp-mbf-logo"><img src="
				<?php
				echo SP_WPSPRO_URL
				?>
					admin/assets/images/wpspro-logo.png" alt="Product Slider
				Pro for WooCommerce"></div>
				<div class="sp-wpsp-mbf-short-links">
			...
			<div class="wpspro_shortcode text-center">
				<div class="wpspro-col-lg-3">
					<div class="wpspro_shortcode_content">
						<h2 class="wpspro-shortcode-title"><?php _e( 'Shortcode', 'woo-product-slider-pro' ); ?> </h2>
						<p><?php _e( 'Copy and paste this shortcode into your posts or pages:', 'woo-product-slider-pro' ); ?></p>
						<div class="spsc-code selectable" >[woo-product-slider-pro <?php echo 'id="' . $post->ID . '"'; ?>]</div>
					</div>
				</div>
				<div class="wpspro-col-lg-3">
					<div class="wpspro_shortcode_content">
						<h2 class="wpspro-shortcode-title"><?php _e( 'Template Include', 'woo-product-slider-pro' ); ?> </h2>

						<p><?php _e( 'Paste the PHP code into your template file:', 'woo-product-slider-pro' ); ?></p>
						<div class="spsc-code selectable">
							&lt;?php
							echo do_shortcode('[woo-product-slider-pro id="<?php echo $post->ID; ?>"]');
							?&gt;</div>
					</div>
				</div>
				<div class="wpspro-col-lg-3">
					<div class="wpspro_shortcode_content">
						<h2 class="wpspro-shortcode-title"><?php _e( 'Post or Page editor', 'woo-product-slider-pro' ); ?> </h2>

						<p><?php _e( 'Insert it into an existing post or page with the icon:', 'woo-product-slider-pro' ); ?></p>
						<img class="tmce-editor-btn" src="<?php echo SP_WPSPRO_URL . '/admin/assets/images/wpspro-editor-button.png'; ?>" alt="">
					</div>
				</div>
			</div>
		...
		if ( $has_nav ) {
			...
			foreach ( $sections as $value ) {
				...
				if ( isset( $value['fields'] ) ) {
					$active_section = ( $section_id == $value['name'] ) ? ' class="sp-section-active"' : 'class="' . $value['name'] . '"';
		...
		foreach ( $sections as $val ) {
			...
			if ( isset( $val['fields'] ) ) {
				...
				foreach ( $val['fields'] as $field_key => $field ) {
					...
					echo sp_wpsp_add_element( $field, $elem_value, $unique );
	...
	public function save_post( $post_id, $post ) {
		...
		if ( wp_verify_nonce( sp_get_var( 'sp-framework-metabox-nonce' ), 'sp-framework-metabox' ) ) {
			...
			set_transient( 'wpsp-metabox-transient', $transient, 10 );
