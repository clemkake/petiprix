 ...
 *
 */
class SP_WPSP_Framework extends SP_WPSP_Framework_Abstract {
	...
	public static function instance( $settings = array(), $options = array() ) {
		if ( is_null( self::$instance ) && SP_WPSP_ACTIVE_FRAMEWORK ) {
	...
	public function validate_save( $request ) {
		...
		if ( isset( $request['resetall'] ) ) {
			$add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'woo-product-slider-pro' ), 'updated' );
			return;
		}
		...
		if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
			...
			$add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'woo-product-slider-pro' ), 'updated' );
		}
	...
	public function field_callback( $field ) {
		...
		echo sp_wpsp_add_element( $field, $value, $this->unique );
	}
	...
	public function admin_page() {
		...
		echo '<div class="sp-wpsp-framework sp-option-framework">';

		echo '<form method="post" action="options.php" enctype="multipart/form-data" id="sp_wpsp_framework_form">';
		...
		echo ( $this->settings['ajax_save'] ) ? '<span id="sp-save-ajax">'. esc_html__( 'Settings saved.', 'woo-product-slider-pro' ) .'</span>' : '';

		submit_button( esc_html__( 'Save Changes', 'woo-product-slider-pro' ), 'primary sp-save', 'save', false, array( 'data-save' => esc_html__( 'Saving...', 'woo-product-slider-pro' ) ) );
		submit_button( esc_html__( 'Restore', 'woo-product-slider-pro' ), 'secondary sp-restore sp-reset-confirm', $this->unique .'[reset]', false );

		if( $this->settings['show_reset_all'] ) {
			submit_button( esc_html__( 'Reset All Options', 'woo-product-slider-pro' ), 'secondary sp-restore sp-warning-primary sp-reset-confirm', $this->unique .'[resetall]', false );
		}

		echo '</fieldset>';
		//echo ( empty( $has_nav ) ) ? '<a href="#" class="sp-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'woo-product-slider-pro' ) .'</a>' : '';
		...
		echo '</div>'; // end .sp-wpsp-framework
