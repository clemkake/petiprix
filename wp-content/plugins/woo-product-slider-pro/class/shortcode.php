<?php
/**
 * The file that defines the slider post type.
 *
 * A class the that defines the slider post type and make the plugins' menu.
 *
 * @link http://shapedplugin.com
 * @since 1.0.0
 *
 * @package Woo_Product_Slider_Pro
 * @subpackage Woo_Product_Slider_Pro/class
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

/**
 * Custom post class to register the slider.
 */
class WPSPRO_Shortcode {

	private static $_instance;

	public function __construct() {
		add_filter( 'init', array( $this, 'register_post_type' ) );
	}

	public static function getInstance() {
		if ( ! self::$_instance ) {
			self::$_instance = new WPSPRO_Shortcode();
		}

		return self::$_instance;
	}

	/**
	 * Register post type.
	 *
	 * @return void
	 */
	public function register_post_type() {

		if ( post_type_exists( 'sp_wpsp_shortcodes' ) ) {
			return;
		}

		// Set the WPSP post type labels.
		$labels = apply_filters(
			'sp_wpspro_post_type_labels',
			array(
				'name'               => esc_html__( 'All Sliders', 'woo-product-slider-pro' ),
				'singular_name'      => esc_html__( 'Product Slider', 'woo-product-slider-pro' ),
				'menu_name'          => esc_html__( 'Product Slider Pro', 'woo-product-slider-pro' ),
				'all_items'          => esc_html__( 'All Sliders', 'woo-product-slider-pro' ),
				'add_new'            => esc_html__( 'Add New', 'woo-product-slider-pro' ),
				'add_new_item'       => esc_html__( 'Add New Slider', 'woo-product-slider-pro' ),
				'edit'               => esc_html__( 'Edit', 'woo-product-slider-pro' ),
				'edit_item'          => esc_html__( 'Edit Slider', 'woo-product-slider-pro' ),
				'new_item'           => esc_html__( 'Product Slider', 'woo-product-slider-pro' ),
				'search_items'       => esc_html__( 'Search Product Sliders', 'woo-product-slider-pro' ),
				'not_found'          => esc_html__( 'No Product Sliders found', 'woo-product-slider-pro' ),
				'not_found_in_trash' => esc_html__( 'No Product Sliders in Trash', 'woo-product-slider-pro' ),
				'parent'             => esc_html__( 'Parent Product Slider', 'woo-product-slider-pro' ),
			)
		);

		// Set the WPSP post type arguments.
		$args = apply_filters(
			'sp_wpspro_post_type_args',
			array(
				'labels'          => $labels,
				'hierarchical'    => false,
				'description'     => esc_html__( 'Product Slider Pro for WooCommerce', 'woo-product-slider-pro' ),
				'public'          => false,
				'show_ui'         => current_user_can('manage_options') ? true : false,
				'show_in_menu'    => current_user_can('manage_options') ? true : false,
				'menu_icon'       => 'data:image/svg+xml;base64,',
				'menu_position'   => 57,
				'query_var'       => false,
				'capability_type' => 'post',
				'supports'        => array(
					'title',
				),
			)
		);

		register_post_type( 'sp_wpsp_shortcodes', $args );

	}

}
